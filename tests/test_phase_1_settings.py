...
def test_direct_settings_construction_rejects_empty_paths(
    field_name: str,
) -> None:
    values = {
        "data_root": Path("runtime/saxophone"),
...
def test_direct_settings_construction_rejects_absolute_parent_traversal(
    field_name: str,
) -> None:
    values = {
        "data_root": Path("runtime/saxophone"),
    ...
    values[field_name] = Path("C:/safe/../outside")
...
def test_from_environment_rejects_absolute_parent_traversal(variable: str) -> None:
    with pytest.raises(SettingsValidationError) as error:
        AppSettings.from_environment({**VALID_ENVIRONMENT, variable: "C:/safe/../outside"})
...
def test_direct_settings_construction_rejects_control_characters_in_paths(
    field_name: str,
) -> None:
    values = {
        "data_root": Path("runtime/saxophone"),
    ...
    values[field_name] = Path("runtime/saxo\x00phone")
...
def test_from_environment_rejects_control_characters_in_paths(
    variable: str,
    field_name: str,
) -> None:
    with pytest.raises(SettingsValidationError) as error:
        AppSettings.from_environment({**VALID_ENVIRONMENT, variable: "runtime/saxo\x00phone"})
...
@pytest.mark.parametrize("device_name", ["CON", "prn.txt", "NUL", "COM1.log", "LPT9"])
def test_direct_settings_construction_rejects_windows_device_path_components(
    field_name: str,
    device_name: str,
) -> None:
    values = {
    ...
    values[field_name] = Path("runtime") / device_name
...
@pytest.mark.parametrize("device_name", ["CON", "prn.txt", "NUL", "COM1.log", "LPT9"])
def test_from_environment_rejects_windows_device_path_components(
    variable: str,
    field_name: str,
    device_name: str,
) -> None:
    with pytest.raises(SettingsValidationError) as error:
        AppSettings.from_environment({**VALID_ENVIRONMENT, variable: f"runtime/{device_name}"})
...
def test_direct_settings_construction_rejects_windows_invalid_path_characters(
    ...
    values[field_name] = Path("runtime") / component
...
def test_from_environment_rejects_windows_invalid_path_characters(
    ...
    with pytest.raises(SettingsValidationError) as error:
        AppSettings.from_environment({**VALID_ENVIRONMENT, variable: f"runtime/{component}"})
