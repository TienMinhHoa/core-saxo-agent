...
def test_asset_route_maps_repository_permission_error_to_forbidden() -> None:
    payload = b"png-bytes"
    artifact = ArtifactRef(
        artifact_id="doc-1/images/page-1.png",
        version="image-v1",
    ...
    class PermissionDeniedArtifacts(FakeArtifacts):
        async def get(self, artifact: ArtifactRef) -> bytes:
    ...
    app = create_app(
        ...
        overrides=AppOverrides(
            remote_gpu_gateway=FakeRemoteGpuGateway(),
            model_client=FakeModelClient(),
            artifact_repository=PermissionDeniedArtifacts(payload),
...
def test_asset_route_maps_directory_read_error_to_not_found() -> None:
    payload = b"png-bytes"
    artifact = ArtifactRef(
        artifact_id="doc-1/images/page-1.png",
        version="image-v1",
    ...
    class DirectoryArtifacts(FakeArtifacts):
        async def get(self, artifact: ArtifactRef) -> bytes:
            raise IsADirectoryError("artifact identity is a directory")
    ...
    app = create_app(
        ...
        overrides=AppOverrides(
            remote_gpu_gateway=FakeRemoteGpuGateway(),
            model_client=FakeModelClient(),
            artifact_repository=DirectoryArtifacts(payload),
            image_artifact_resolver=FakeImageArtifactResolver(artifact, []),
        ),
    )
...
def test_asset_route_maps_non_directory_path_error_to_not_found() -> None:
    payload = b"png-bytes"
    artifact = ArtifactRef(
        artifact_id="doc-1/images/page-1.png",
        version="image-v1",
    ...
    class NonDirectoryPathArtifacts(FakeArtifacts):
        async def get(self, artifact: ArtifactRef) -> bytes:
    ...
    app = create_app(
        ...
        overrides=AppOverrides(
            remote_gpu_gateway=FakeRemoteGpuGateway(),
            model_client=FakeModelClient(),
            artifact_repository=NonDirectoryPathArtifacts(payload),
            image_artifact_resolver=FakeImageArtifactResolver(artifact, []),
        ),
    )
...
def test_asset_route_maps_missing_artifact_to_not_found() -> None:
    payload = b"png-bytes"
    artifact = ArtifactRef(
        artifact_id="doc-1/images/page-1.png",
        version="image-v1",
    ...
    class MissingArtifacts(FakeArtifacts):
        async def get(self, artifact: ArtifactRef) -> bytes:
    ...
    app = create_app(
        ...
        overrides=AppOverrides(
            remote_gpu_gateway=FakeRemoteGpuGateway(),
            model_client=FakeModelClient(),
            artifact_repository=MissingArtifacts(payload),
            image_artifact_resolver=FakeImageArtifactResolver(artifact, []),
        ),
    )
