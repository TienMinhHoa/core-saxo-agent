...
def test_backend_dependency_manifests_do_not_lock_gpu_runtime() -> None:
    """Permit the optional CPU layout client but never lock a GPU runtime."""
...
def test_readme_identifies_saxophone_api_as_primary_backend() -> None:
    ...
    assert "Entrypoint duy nhất là\n`saxophone-api`" in readme
...
def test_backend_package_does_not_import_legacy_runtime_modules() -> None:
    ...
    violations = {
        ...
        for path in SOURCE_ROOT.rglob("*.py")
        if _import_roots(path) & LEGACY_MODULE_ROOTS
    }
...
def test_packaging_does_not_reintroduce_removed_legacy_extracted_package() -> None:
    ...
    assert package_includes == {"saxophone*"}
    assert not any(pattern.startswith("extracted") for pattern in package_includes)
